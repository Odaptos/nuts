...
Nuts.prototype.onUpdate = function(req, res, next) {
    ...
    .then(function(versions) {
        ...
        var returnUrl = urljoin(fullUrl, gitFilePath, '/download/version/'+latest.tag+'/'+platform);
       ...
       .then(function(content) {
            ...
            releases = _.chain(releases)
                ...
                .map(function(entry) {
                    ...
                    console.log('Parsed Windows Update URL: Before ---> ', urljoin(fullUrl, gitFilePath, '/download/'+entry.semver+'/'+entry.filename));
                    console.log('Parsed Windows Update URL: After ---> ', urljoin(fullUrl.replace(url.parse(fullUrl).search, ''), gitFilePath, '/download/'+entry.semver+'/'+entry.filename));
