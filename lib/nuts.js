...
Nuts.prototype.onUpdate = function(req, res, next) {
    ...
    .then(function(versions) {
        ...
        var filePath = '/download/version/'+latest.tag+'/'+platform+'?filetype='+filetype;

        console.error(platform);
        // if (['osx_64', 'darwin_64', 'mac_64'].includes(platform)) {
        //     filePath += '?filetype='+filetype;
        // }

        res.status(200).send({
            "url": urljoin(fullUrl, gitFilePath, filePath),
            "name": latest.tag,
            "notes": releaseNotes,
